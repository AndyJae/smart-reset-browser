"""
camera_plugins/panasonic/aw_he130.py — Panasonic AW-HE130

Professional HD integrated PTZ camera.
...
RESET_COMMANDS = [
    # White balance ATW targets (applied while ATW is active)
    ...
    ("BLACK GAMMA RANGE",                "OSJ", "1B", "1"),    # 0=LOW 1=MID 2=HIGH
    # Matrix — default NORMAL; USER mode is forced during reset sequence for elements
    ...
    # Knee — enabled only long enough to set levels, then disabled
    ("KNEE MODE",                        "OSA", "2D", "1"),
    ("KNEE POINT",                       "OSA", "20", "80"),
    ("KNEE SLOPE",                       "OSA", "24", "63"),
    ("KNEE MODE",                        "OSA", "2D", "0"),
    # White clip — enabled only long enough to set level, then disabled
    ("WHITE CLIP",                       "OSA", "2E", "1"),
    ...
    # 12-zone colour correction — saturation (applied when MATRIX TYPE=USER)
    ...
    # Skin tone detail — enabled only to set level, then disabled
...
def _run_reset_sequence(context, _prep: dict) -> None:
    ...
    # Apply both linear matrix elements and colour correction zones under MATRIX TYPE=USER
