"""
camera_plugins/panasonic/aw_ue80.py — Panasonic AW-UE80

Migrated from camera_types/camera_aw_ue80.py.
...
def _run_reset_sequence(context, prep: dict) -> None:
    ...
    if atw:
        atw_ready = _ensure_dropdown_value(
            context, "WHITE BALANCE MODE", "QAW", "OAW", "0"  # ATW
        )
        if atw_ready:
    ...
    if color_correct:
        matrix_ready = _ensure_dropdown_value(
            context, "MATRIX TYPE", "QSE:31", "OSE:31", "3"  # USER
        )
        if matrix_ready:
            ...
            for entry in knee:
                ...
                if not ok:
                    context.logging.warning(
                        f"Skipping {entry['label']}: write not accepted in current camera mode"
                    )
        else:
