"""
camera_plugins/birddog/p110.py — BirdDog P110 / P120 reset module.
...
# P110/P120 supports 12 colour matrix params (0-64).
...
def run_reset(context) -> None:
    ...
    for label, path, defaults in sections:
        context.logging.info(f"BirdDog P110/P120: resetting {label}...")
        ok = send_post(context, label, path, defaults)
        if ok:
            context.logging.info(f"BirdDog P110/P120: {label} OK.")
        else:
            context.logging.warning(f"BirdDog P110/P120: {label} failed — no response.")
