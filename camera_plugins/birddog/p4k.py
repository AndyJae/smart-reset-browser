"""
camera_plugins/birddog/p4k.py — BirdDog P4k / P400 reset module.
...
NOTE: /birddogcmsetup is N/A on P4k/P400 — colour matrix is not supported.
"""
...
CAMERA_ID = "P4K"
CAMERA_ID_ALIASES = ["P4K", "P4k", "BirdDog P4K", "BirdDog P4k", "P400", "BirdDog P400"]
DISPLAY_NAME = "BirdDog P4k/P400"
...
def run_reset(context) -> None:
    """Resets all image settings to defaults. Colour matrix is N/A on P4k/P400."""
    import json
    ...
    sections = [
        ...
        # ColourMatrix (/birddogcmsetup) is N/A on P4k/P400 — skipped.
    ]
    for label, path, defaults in sections:
        context.logging.info(f"BirdDog P4k/P400: resetting {label}...")
        ok = send_post(context, label, path, defaults)
        if ok:
            context.logging.info(f"BirdDog P4k/P400: {label} OK.")
        else:
            context.logging.warning(f"BirdDog P4k/P400: {label} failed — no response.")
