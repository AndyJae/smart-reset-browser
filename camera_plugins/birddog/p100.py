"""
camera_plugins/birddog/p100.py — BirdDog P100 / PF120 reset module.
...
Reset sections:
  ...
  POST /birddogcmsetup      — Colour Matrix (12 params, no ColourGain/HuePhase)
"""
...
CAMERA_ID = "P100"
CAMERA_ID_ALIASES = ["P100", "BirdDog P100", "PF120", "BirdDog PF120"]
DISPLAY_NAME = "BirdDog P100/PF120"
# PROTOCOL imported from base ("birddog")

RESET_COMMANDS: list = []

# ---------------------------------------------------------------------------
# UI definitions (same controls as P200, minus wb_trigger condition on One Push)
# ---------------------------------------------------------------------------
...
# P100/PF120 supports 12 colour matrix params (0-64).
# ColourGain and HuePhase are P200/A200/A300 only — omitted here.
_CM_DEFAULTS = {
    "BlueGain":   "32",
    "BlueHue":    "32",
    "CyanGain":   "32",
...
def run_reset(context) -> None:
    ...
    for label, path, defaults in sections:
        context.logging.info(f"BirdDog P100/PF120: resetting {label}...")
        ok = send_post(context, label, path, defaults)
        if ok:
            context.logging.info(f"BirdDog P100/PF120: {label} OK.")
        else:
            context.logging.warning(f"BirdDog P100/PF120: {label} failed — no response.")
