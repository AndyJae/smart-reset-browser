"""
camera_plugins/birddog/p_generic.py — Generic BirdDog fallback reset module.

Used when a BirdDog camera is found on the network but its DeviceID is not
recognised by any specific module. Resets only the sections common to all
BirdDog models. Colour matrix is skipped — support varies by model.
"""
...
CAMERA_ID_ALIASES: list = []
DISPLAY_NAME = "BirdDog (unknown model)"
...
# All known colour matrix params across all BirdDog models.
# The camera will accept what it supports and ignore or reject the rest —
# send_post handles failures gracefully and moves on.
_CM_DEFAULTS = {
    "BlueGain":   "32",
    "BlueHue":    "32",
    "CyanGain":   "32",
...
def run_reset(context) -> None:
    """Resets all BirdDog sections. Each section is attempted regardless of
    the previous result — the camera accepts what it supports and rejects the rest."""
    import json
    ...
    for label, path, defaults in sections:
        context.logging.info(f"BirdDog (generic): resetting {label}...")
        ok = send_post(context, label, path, defaults)
        if ok:
            context.logging.info(f"BirdDog (generic): {label} OK.")
        else:
            context.logging.warning(f"BirdDog (generic): {label} failed — no response.")
